...
if ~flag0
    ...
    fopt = optimset('Display', 'off', 'TolX', 1e-12, 'TolFun', 1e-12);
    [x, ~, exitflag] = fsolve(@(x) tankc0_res(x, MC, P_alpha, P_gamma, P_varphi, P_tau_d, P_lambda), [N0; CH0], fopt);
    if exitflag <= 0; check = 1; return; end
    N   = x(1);
    C_H = x(2);
    W   = (1 - P_alpha) * MC * N^(-P_alpha);
    Y   = N^(1 - P_alpha);
